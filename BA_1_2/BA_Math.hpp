...
public:
	...
	BA_Array(_ULL* _dataShape, _ULL _shapeLen, _ULL content);
	BA_Array(_ULL* _dataShape, _ULL _shapeLen, float content);
...
LOFE_Model* LOFE_BindLayers(LOFE_Model* model, _ULL layer1index, _ULL layer2index);//from1  layer1->layer2
...
LOFE_Chain* LOFE_CompileModel_B_PutB(LOFE_Chain* chain, float* pw, float* pb, _ULL k, _ULL* bias);//k为sumlayer-层序号(from1)
...
LOFE_Layer* LOFE_Relu(LOFE_Layer* layer);//  or /*y = x + |x|*/
...
LOFE_Layer* LOFE_FillLayer(LOFE_Layer* pl, _LL sum, ...);//sum<0 means a float array which is size of -sum

LOFE_Layer* LOFE_Predict(LOFE_Model* model);//仅前向传播

LOFE_Model* LOFE_Loss(LOFE_Model* model);//计算loss

LOFE_Layer* LOFE_Train(LOFE_Model* model);//将各函数集合，计算单个样本并进行梯度下降
LOFE_Model* LOFE_Train_ApllyG(LOFE_Model* model);//应用梯度
LOFE_Model* LOFE_Train_CacuChain(LOFE_Model* model);//计算所有梯度
float LOFE_Train_CacuSubChain(LOFE_Model* model, LOFE_Tensor* pt, float* p, bool isw, _ULL k);//引用并返回单个w or b涉及的梯度
